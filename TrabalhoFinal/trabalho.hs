...
--print(ordenaListaPelaPrioridade [("Tarefa A", 1, 1), ("Tarefa X", 2, 2), ("Tarefa ¨6", 7, 3), ("Tarefa K", 4, 5)])
...
geraPosicaoDosItens :: [([Char],Int)] -> Int -> [([Char],Int,Int)]
geraPosicaoDosItens [] = []
geraPosicaoDosItens ((a,b):xs) pin = (a,b,pin) : geraPosicaoDosItens xs (pin+1)
...
atualizaPrioridade ((a,b,c):xs) n = if b == n then (a,b,c) : atualizaPrioridade xs m else (a, n+1, c) : atualizaPrioridade xs (n+1)
...
ordenaListaPelaOrdem ((a,b,c):xs) = if c <= menor_ordem ((a,b,c):xs) then (a,b,c) : ordenaListaPelaOrdem xs else ordenaListaPelaOrdem(xs ++ [(a,b,c)]
